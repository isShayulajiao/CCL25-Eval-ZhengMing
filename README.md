...
#### **Weighted F1-score**
**Weighted F1-score** 是精确率 (Precision) 和召回率 (Recall) 的调和平均，用于综合评估模型在识别情感正例上的表现，尤其适用于类别不平衡的情感分析任务。它能够在精确率和召回率之间取得平衡，体现模型识别正面或负面情感的整体效果。
...
在情感倾向分析中，Weighted F1-score帮助评估模型识别特定情感（如正面或负面）的精度和完整性。较高的Weighted F1-score表明模型不仅能够准确地识别情感类型，还能覆盖尽可能多的情感样本。

**评价指标的说明：**
- **高 Weighted F1-score** 表示模型能有效、全面地识别出评论中的情感倾向。
- **低 Weighted F1-score** 则可能表明模型在情感识别上存在遗漏或误判。
...
**公式符号解释**
- $N$ 是类别的数量。
- $F1_i$ 是第 $i$ 类别的Weighted F1-score。
...
![文学命名实体识别（LitNRE）数据样例](static/LitNRE_sample.jpg)
...
点击报名链接填写报名表 https://f.wps.cn/g/BYw3bQEo/ 进行报名，我们会发送邮件确认您已经参赛。本次评测提供的**ZhengMing评测基准**的[语料](https://open.virtaicloud.com/web/data/detail/548421738158239744)以及[代码](https://open.virtaicloud.com/web/project/detail/548005418581241856)均放置在**驱动云**AI训练平台上。  
